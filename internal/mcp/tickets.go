...
type ListTicketsInput struct {
	...
	Today       bool   `json:"today,omitempty" jsonschema:"only tickets planned for today plus active work"`
}
...
type SubmitTicketInput struct {
	...
	Summary  string `json:"summary" jsonschema:"concise implementation and verification summary"`
}
...
func ListTicketsHandler(input ListTicketsInput) string {
	...
	for _, ticket := range all {
		...
		if input.Today && ticket.PlannedFor != today && ticket.Status != tickets.StatusRunning && ticket.Status != tickets.StatusReview {
			continue
		}
...
func submitTicket(store *tickets.Store, mySession string, input SubmitTicketInput) string {
	...
	updated, err := store.Update(ticket.ID, func(current *tickets.Ticket) error {
		...
		current.SubmittedSummary = summary
		return nil
	})
	if err != nil {
		return "Error: " + err.Error()
